...
int main(void)
{
    ...
    /* A nonzero begin must not create a zero-time motion jump. */
    count = 0;
    cocoa_pinch_event(&pinch, COCOA_PINCH_BEGIN, true, -0.25, record, frames);
    assert(count == 1 && frames[0].down);
    assert(frames[0].x[1] == COCOA_PINCH_CENTER + 3072);
    cocoa_pinch_event(&pinch, 0, true, 0.5, record, frames);
    assert(count == 2 && frames[1].down);
    assert(frames[1].x[1] == COCOA_PINCH_CENTER + 4608);
    cocoa_pinch_event(&pinch, COCOA_PINCH_END, true, 0, record, frames);
    assert(count == 3 && !frames[2].down);
    /* A rapid repeat gets one initial position, with a fresh scale. */
    cocoa_pinch_event(&pinch, COCOA_PINCH_BEGIN, true, 0.25, record, frames);
    assert(count == 4 && frames[3].down);
    assert(frames[3].x[1] == COCOA_PINCH_CENTER + 5120);
    cocoa_pinch_cancel(&pinch, record, frames);
    assert(count == 5 && !frames[4].down);
    return 0;
}
''')
